...
function menuIconToggle(){
    ...
    for (i = 0; i < icons.length; i++){
        if(icons[i].innerHTML == 'menu'){
            icons[i].classList.add('shoink');
            let actualIcon = icons[i];
            setTimeout(function(actualIcon){
                actualIcon.innerHTML = 'close';
                actualIcon.classList.remove('shoink');
            }, 125, actualIcon);
